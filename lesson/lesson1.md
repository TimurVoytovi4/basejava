...
### Optional
- Модифицировать класс `webapp.storage.ArrayStorage`: хранить все резюме в начале webapp.storage (без дырок null), чтобы не перебирать каждый раз все 10000 элементов.
```
Хранеие резюме в webapp.storage (от 0 до size-1 элементов null нет):

r1, r2, r3,..., rn, null, null,..., null
<----  size ----->
<----  webapp.storage.length (10000)---------->
```
