...
> Правка к видео: webapp.storage.ArrayStorage.delete() - вместо `webapp.storage[i] = null` нужно `webapp.storage[size-1] = null`
...
- Сделать проверки: в `update/delete/get` - резюме есть в webapp.storage, в `save`- нет в webapp.storage: `System.out.println("webapp.model.Resume ...")`.
